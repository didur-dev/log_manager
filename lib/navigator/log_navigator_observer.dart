...
class LogNavigatorObserver extends NavigatorObserver {
  ...
  void _sendRouteStart(Route<dynamic> route, Route<dynamic>? previousRoute) {
    final String? screenName = nameExtractor(route);
    String? namePrevious;
    if (screenName != null) {
      if (previousRoute != null) {
        namePrevious = nameExtractor(previousRoute);
      }

      Log.analytics
          .routeStart(name: screenName, namePrevious: namePrevious)
  ...
  void _handleAnalyticsErrors(Object error) {
    ...
    if (onError == null) {
      debugPrint('$LogNavigatorObserver: $error');
    } else {
  ...
  void didPush(Route<dynamic> route, Route<dynamic>? previousRoute) {
    ...
    if (routeFilter(route)) {
      _sendRouteStart(route, previousRoute);
    }
  }
  ...
  void didReplace({Route<dynamic>? newRoute, Route<dynamic>? oldRoute}) {
    ...
    if (newRoute != null && routeFilter(newRoute)) {
      _sendRouteStart(newRoute, oldRoute);
    }
  }
  ...
  void didPop(Route<dynamic> route, Route<dynamic>? previousRoute) {
    ...
    if (previousRoute != null && routeFilter(previousRoute) && routeFilter(route)) {
      _sendRouteStart(previousRoute, previousRoute);
    }
  }
}
